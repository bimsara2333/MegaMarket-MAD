...
class UpdateDataActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val cat=findViewById<EditText>(R.id.cat)
        val name=findViewById<EditText>(R.id.name)
        val qty=findViewById<EditText>(R.id.qty)
        val desc=findViewById<EditText>(R.id.desc)
        val price=findViewById<EditText>(R.id.price)
        val updateBtn=findViewById<Button>(R.id.update)

        var id = intent.getStringExtra("empId").toString()

        database.child("Employees").child(id).addListenerForSingleValueEvent(object :
            ValueEventListener {
